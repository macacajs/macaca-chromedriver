...
function getOsMapChromeVsersion() {
  ...
  if (platform === 'darwin') {
    tag = true;
    const CDNURL = process.env.CHROMEDRIVER_CDNURL_AUTO || 'http://chromedriver.storage.googleapis.com';
    versionNo = child_process.spawnSync(path, ['--version']);
    shiftVersion = versionNo.output.toString().split(' ')[2].toString().split('.')[0];
    version = chromedriverVersions.getVersionFromWebviewVersion(shiftVersion);
