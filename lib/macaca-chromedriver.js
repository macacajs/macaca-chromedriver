...
const { install, getInstalledChromeVersion } = require('./install');
const Versions = require('./versions');
const DriverProxy = require('./proxy');

// Deprecated
const fileName = _.platform.isWindows ?
  'chromedriver' + Versions.defaultVersion + '.exe' :
  'chromedriver' + Versions.defaultVersion;
// Deprecated
...
class ChromeDriver extends EventEmitter {
  ...
  getChromedriverVersion() {
    const version = getInstalledChromeVersion();
    if (process.env.CHROMEDRIVER_VERSION) {
      ...
      logger.info('if you don\'t want to use this version, please remove your process.env.CHROMEDRIVER_VERSION ');
      return process.env.CHROMEDRIVER_VERSION;
    } else if (process.env.WEBVIEW_VERSION) {
      ...
      logger.info(`Select the automatic download version ${version}`);
      return version;
    } else {
