...
class ChromeDriver extends EventEmitter {
  ...
  killAll() {
    ...
    let cmd;
    if (_.platform.isOSX) {
      cmd = `ps -ef | grep chromedriver | grep -v grep | grep -e '--port=${this.proxyPort}' | awk '{ print $2 }' | xargs kill -15`;
    } else if (_.platform.isLinux) {
      cmd = `ps -ef | grep chromedriver | grep -v grep | grep -e '--port=${this.proxyPort}' | awk '{ print $2 }' | xargs -r kill -15`;
